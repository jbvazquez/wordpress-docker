...
class EPKB_New_Features_Page {
	...
	private static function features_list( $features=array() ) {
		...
		$features['2021.02.01'] = array(
			...
			'learn_more_url'    => EPKB_Utilities::get_one_editor_url( 'article_page', 'print_button' ),
	   ...
	   $features['2021.01.02'] = array(
		   ...
		   'learn_more_url'    => EPKB_Utilities::get_one_editor_url( 'article_page', 'article_content' ),
		   'button_name'       => __( 'Try Now!', 'echo-knowledge-base'),
		   'plugin-type'       => 'core',
		   'type'              => 'new-feature'
	   );
		...
		$features['2020.12.23'] = array(
			...
			'learn_more_url'    => EPKB_Utilities::is_advanced_search_enabled_simple() ? EPKB_Utilities::get_one_editor_url( 'article_page', 'search_box_zone' ) : '',
		   'button_name'       => __( 'Try Now!', 'echo-knowledge-base'),
			'plugin-type'       => 'add-on',
			'type'              => 'new-feature'
		);
		...
		$features['2020.11.01'] = array(
			...
			'learn_more_url'    => EPKB_Utilities::get_one_editor_url( 'main_page' ),
			'plugin-type'       => 'core',
			'type'              => 'new-feature',
			'button_name'       => __( 'Try Now', 'echo-knowledge-base'),
		);
	...
	private static function get_new_features_list() {
		...
		foreach ( $features as $date => $feature ) {
			...
			if ( ! empty($season[0]) && ! empty($season[1]) ) {
				$key = $season[0] == date('Y') ? self::group_by_month( $season[1] ) : __( 'Year' ) . ' ' . $season[0];
